/*
 * eyes.c — a native Wayland "eyes" app compiled to WebAssembly, running
 * directly in the browser inside the Greenfield compositor (web:// mode, no
 * No proxy, no h264 — pure wl_shm + xdg-shell + wl_pointer.
 *
 * It hand-rasterizes two eyes into a shared-memory buffer (2x2 supersampled for
 * anti-aliasing) whose pupils follow the pointer. Per the Wayland security
 * model a client only gets pointer events while the cursor is over its own
 * surface, so the eyes track the cursor when it is over the window.
 */
#include <math.h>
#include <stdbool.h>
#include <stdint.h>
...
int main(void)
{
	...
	// Force the emscripten runtime to actually exit so the SDK's Module.onExit
	// fires and posts 'Terminate' to the compositor (ASYNCIFY/pthreads otherwise
	// keep the runtime alive after main returns, so the window would never close).
