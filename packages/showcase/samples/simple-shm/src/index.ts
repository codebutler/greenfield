...
/* Dedicated wl_pointer listener. It must be separate from Display because
 * Display is the wl_keyboard listener and wl_keyboard/wl_pointer share the
 * event names enter/leave (the runtime dispatches by event name). */
class PointerHandler implements WlPointerEvents {
  constructor(private readonly display: Display) {}

  button(serial: number, time: number, button: number, state: number): void {
    if (button === BTN_LEFT && state & WlPointerButtonState._pressed && this.display.activeToplevel && this.display.seat) {
      this.display.activeToplevel.move(this.display.seat, serial)
    }
  }
  enter(serial: number, surface: WlSurfaceProxy, surfaceX: Fixed, surfaceY: Fixed): void {}
...
class Display implements ClientDisplay, WlRegistryEvents, WlShmEvents, XdgWmBaseEvents, WlSeatEvents {
  ...
  hasXRGB = false
  keyboard?: WlKeyboardProxy
  pointer?: WlPointerProxy
  pointerHandler = new PointerHandler(this)
  seat?: WlSeatProxy
  shm?: WlShmProxy
  wmBase?: XdgWmBaseProxy
  // the toplevel to move when the window body is pressed (Wayland has no
  // server-side title bars; the client must request the interactive move)
  activeToplevel?: XdgToplevelProxy
  ...
  capabilities(caps: number): void {
    ...
    if (caps & WlSeatCapability._pointer && this.pointer === undefined) {
      this.pointer = this.seat!.getPointer()
      this.pointer.listener = this.pointerHandler
    } else if (!(caps & WlSeatCapability._pointer) && this.pointer) {
...
async function main() {
  ...
  // let pointer presses on this window's body start an interactive move
  display.activeToplevel = window.xdgToplevel
